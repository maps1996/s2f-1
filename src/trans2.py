...
class material:
    ...
    def converse(self):
        ...
        for i in range(ng):
            for j in range(ng):
                self.p0_c[i][j] = float(self.p0[j + nup - i][j])
                self.p1_c[i][j] = float(self.p1[j + nup - i][j])
                self.p2_c[i][j] = float(self.p2[j + nup - i][j])
                self.p3_c[i][j] = float(self.p3[j + nup - i][j])
                self.p4_c[i][j] = float(self.p4[j + nup - i][j])
                self.p5_c[i][j] = float(self.p5[j + nup - i][j])
        for i in range(ng):
